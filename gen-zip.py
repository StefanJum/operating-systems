...
def main():
    if len(sys.argv) != 2:
        # print(f"Usage: {sys.argv[0]} <lab_structure_yaml>")
        sys.exit(1)
    ...
    for lab_title, info in lab_info.items():
        ...
        for stem in info['stems']:
            ...
            if found_dirs:
                # print(f"  📁 Directories for '{stem}':")
                # for d in found_dirs:
                    # print(f"    - {d}")
                dirs_to_zip.extend(found_dirs)
            # else:
                # print(f"  ⚠️ No directory found for '{stem}'")

        if dirs_to_zip:
            create_zip_archive(lab_title, dirs_to_zip)
        # else:
            # print(f"⚠️ No directories found to zip for {lab_title}")
